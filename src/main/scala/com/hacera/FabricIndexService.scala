...
import com.daml.ledger.participant.state.index.v1.impl.reference._
...
final case class FabricIndexService(
  ...
  object StateController {
    ...
    private val currentState: AtomicReference[FabricIndexState] = new AtomicReference(
      FabricIndexState.initialState(initialConditions)
    )

    def updateState(f: FabricIndexState => FabricIndexState): Unit = {
      currentState.getAndUpdate(f(_))
      stateChangeDispatcher.signal()
    }

    def getState: FabricIndexState = currentState.get

    // Subscribe to stream of new states. Does not emit a state until it
    // has been initialized (e.g. first update has been processed).
    def subscribe(): Source[FabricIndexState, NotUsed] =
  ...
  private def futureWithState[T](handler: FabricIndexState => Future[T]): Future[T] = {
  ...
  override def listPackages(): Future[List[PackageId]] =
    futureWithState { state =>
      Future.successful(fabricConn.getPackageList().map(entry => Ref.PackageId.assertFromString(entry)).toList)
    }

  override def isPackageRegistered(packageId: PackageId): Future[Boolean] =
    futureWithState { state =>
      Future.successful(fabricConn.getPackageList().contains(packageId.toString))
    }

  override def getPackage(packageId: PackageId): Future[Option[DamlLf.Archive]] =
    futureWithState { state => {
        val pkgBytes = fabricConn.getPackage(packageId.toString)
        if (pkgBytes == null) {
          return Future.successful(Option[DamlLf.Archive](null))
        }
        return Future.successful(
          Option(DamlLf.Archive.parseFrom(pkgBytes))
        )
      }
    }
